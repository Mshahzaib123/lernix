...
const CourseCardL = ({ data }) => {
    return (
        <div className="grid sm:grid-cols-[auto_1fr] items-center gap-6 xl:p-7 p-6 bg-white rounded-lg border-b-3 border-b-solid border-b-transparent shadow-boxShadow duration-300 group hover:-translate-y-2.5 hover:border-b-primary">
            <Link to={`/courses-detail/${data.id}`} className="overflow-hidden rounded">
                <img className="sm:w-40 w-full h-40 object-cover duration-300 group-hover:scale-110" src={data.img} alt="" />
            </Link>
                ...
                <div className="flex justify-between items-center">
                    ...
                    <ul className="flex items-center gap-0.5">
                        {data.ratingStar.map((item, index) => (
                            <li key={index}>
                                <item.icon className='text-yellow' weight={item.style} size={18} />
                            </li>
                        ))}
                    </ul>
                </div>
