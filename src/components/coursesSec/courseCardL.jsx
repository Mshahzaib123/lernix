...
const CourseCardL = ({ data }) => {
    return (
        <div className="grid sm:grid-cols-[auto_1fr] items-center gap-6 xl:p-7 p-6 bg-white rounded-lg border-b-3 border-b-solid border-b-transparent shadow-boxShadow duration-300 group hover:border-b-primary">
            <Link to={`/courses-detail/${data.id}`} className="overflow-hidden rounded">
                <img className="sm:w-40 w-full h-40 object-cover duration-500 group-hover:scale-105" src={data.img} alt="" />
            </Link>
